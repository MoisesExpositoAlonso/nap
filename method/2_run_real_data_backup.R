...
G<-attach.big.matrix("databig/genome.desc")


####************************************************************************####
gammas<-  .read_gemma(folder="output",name=pheno, what = "bslmm")
gammas$rs<-paste(gammas$chr,gammas$pos,sep="_")
...
alphas<-rep(0,ncol(G))
alphas[mapping]<-gammas$alpha
betas<-rep(0,ncol(G))
betas[mapping]<-gammas$beta

pri<-rep(0,ncol(G))
pri[mapping]<-gammas$gamma


# Define SNPs to analyse based on inclusion prob
l<-1000
m<-which(rank(1-pri) <= l )
pri[m]
bslmm[m] %>%summary
...
hall<-1:nrow(G)
h=sample(1:nrow(G),size = round(0.8 *nrow(G)),replace = F) %>% sort
htest=(1:nrow(G))[-h]
ytrain<-y[h]
ytest<-y[-h]
...
save_plot(paste0("figs/",pheno,"_indplot_,",l,".pdf"),
          plotresults,base_height = 2*5,base_width = 5)


# r<-optim(fn=function(...){lik.nap(...,debug = F)},
#       y=y,
#       h=hall,
#       m=m,
#       A=G,
